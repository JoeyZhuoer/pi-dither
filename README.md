...
- **Water-surface background:** the stopped pattern keeps its original random-like vibration in place. While an agent is thinking or producing output, random droplets rain onto a simulated water surface: each ring interferes with the others and bounces off the window edges, and the ripples are drawn as dithered crests and troughs. Output rains more energetically than thinking. When generation stops, no new droplets are added and the waves slowly settle back to the plain pattern. Driven by real agent activity; automatically paused in hidden tabs or with reduced motion. Pause/resume manually through **? → Background motion**; the preference is remembered.
