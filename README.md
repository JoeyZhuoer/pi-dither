...
- **Background and photo:** the desktop is a flat dusty-pink ground with no animated pattern. The **Background** window lets you pick a ground color and choose a photo; the photo is downscaled, stored locally, and rendered as an ordered dither in the same ink as the rest of the desktop, so it survives reloads. "Remove photo" and "Default colour" restore the plain ground. Nothing is uploaded. Aggregated fleet activity is still reported to the UI (`#desktop[data-activity]`) and drives the activity window, but it paints nothing by itself.
