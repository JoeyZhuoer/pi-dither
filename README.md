...
- **Flowing background:** the stopped pattern stays exactly as it always was, shimmering gently in place. While an agent is thinking or producing output, the dithered field glides slowly and continuously—thinking drifts diagonally, output rises—so the flow is visible while each frame changes only a little. It is driven by real agent activity and automatically paused in hidden tabs or with reduced motion. Pause/resume manually through **? → Background motion**; the preference is remembered.
