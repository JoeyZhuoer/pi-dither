...
- **Laptop motion (opt-in):** the Appearance window can turn the Mac's accelerometer and gyroscope into cloud physics — the cloud leans with gravity, lags behind a push and bursts on a knock. **Re-zero** levels the current pose, and the status line honestly reports `unavailable` when no sensor is present. Off by default.
