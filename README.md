...
- **One-way dither wave:** the stopped pattern keeps its original random-like vibration and stays exactly where it is—no snapping back. While an agent is thinking or producing output, the dithered wave slides steadily in a single direction (~21–28 px/s) with no vibration of its own; when the model stops, it freezes at that position and the idle vibration resumes. Driven by real agent activity; automatically paused in hidden tabs or with reduced motion. Pause/resume manually through **? → Background motion**; the preference is remembered.
