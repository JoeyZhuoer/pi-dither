...
- **Appearance and photo:** the **Appearance** window holds the theme (chrome) colour, the ground colour and an optional photo. The photo is downscaled, stored locally, and rendered as an ordered dither in the same ink as the rest of the desktop, so it survives reloads; moving the pointer across the dots spreads them inside a small radius and they settle back exactly. "Remove photo" and the two default buttons restore the plain look. Nothing is uploaded. Aggregated fleet activity is still reported to the UI (`#desktop[data-activity]`) and drives the activity window, but it paints nothing by itself.
...
- **Dot interaction:** moving the pointer across a dithered photo stirs it in the reference-field style: dots within 26px swirl and spread, faint constellation lines fan out to distinct dots within 90px, and everything settles back to the original pattern over about a second.
- **Particle field (optional):** the Appearance window adds a layer over the desk: **Sparse / Normal / Dense** drifting particles that link with faint lines, are stirred by the pointer and flash on collisions, or **Photo cloud (push/pull)**, which turns your background photo into a point cloud using the reference site's motion—a spring back to each point's home position plus a signed 1/(1+d)² pointer force. It pauses when the tab is hidden and draws a single static frame under reduced motion.
