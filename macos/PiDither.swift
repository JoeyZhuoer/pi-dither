    ...
    func runSmokeStage(_ stage: String) {
        ...
        webView.callAsyncJavaScript("return await piDitherSmoke(stage)", arguments: ["stage": stage], in: nil, in: .page) { result in
            ...
            if let (name, size) = next {
                ...
                print("PASS: native feature checks — nine utilities, roomy opening with a narrow main floor, native resize, manual layouts, maximize, draft controls, menus, appearance colours, dithered photo with pointer ripple, window settings, hide/reopen, Reload and unchanged conversation.")
