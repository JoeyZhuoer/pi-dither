...
This clears only the native `PiDitherMain` saved frame, `pi-desktop:layout:v1` and session-storage observer-dismissal keys prefixed `pi-desktop:delegated-closed:v1:`. The page reset runs before UI initialization at the exact private local origin; its script is removed after that first load, so Reload and later ordinary launches preserve newly chosen geometry. It never clears the whole website store or changes conversations, credentials or workspace preferences. Normal app relaunch still starts a fresh main session; saved conversations remain available in Sessions.
