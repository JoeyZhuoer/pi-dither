...
The background keeps its original idle field while stopped, shimmering in place with the same random-like dither vibration. When the model is thinking or producing output, a dithered wave travels across the field: its direction, wavelength, ripple and speed come from a local fingerprint of the generated text, and the wave also advances as more text arrives. A short hash of the newest generated text (first characters for the shape, length for progress) stays in the page; the text itself is never stored or sent. Output takes priority across connected manual agents and current-session extension children; tools/unknown busy work share the thinking wave without claiming verified model reasoning. Settled/disconnected records do not animate as active work. Activity changes do not override any pause setting.

The background animates at a capped ~12fps, with at most 100,000 drawing cells and a 2048px bitmap edge. Idle keeps its original slow shimmer and hard lattice. Busy states combine a gentle whole-field drift (~1.6 cells/second) with a text-seeded traveling dither wave, a slow amplitude vibration and the same random-like shimmer; typically 1–3% of pixels change per frame, and the threshold is sampled in the moving coordinates so whole-cell crossings never re-dither the field. Hidden tabs/pagehide, system reduced-motion settings and manual pause stop its animation timer. Use **? → Pause background motion** to pause; the preference persists across reloads. Reduced motion takes priority.
