...
The photo is sampled cover-fit at **one cell per CSS pixel**, bounded to 4,000,000 cells and a 4096px edge, and decimated into at most **400,000** points drawn as **1px** dots straight into one reused ImageData buffer (about 6.1 ms median per frame measured for a 386k-point cloud at 1440×900, and 2.8 ms for 193k; the typed-array pool is ~12 MB). The only background-pointer interaction is the reference field's: each point eases toward its home with `1/speed` (speed 20–30) while the pointer adds a **windowed and stiffness-capped** force: the site's `strength / (1 + d)²` is multiplied by `(1 − d/480)²`, is exactly zero for `d ≥ 480px`, and its per-pixel stiffness is capped at 1.5 (the site's term is 10–40× stiffer than the spring at the cursor, which leaves the gather oscillating forever). The whole step is then damped by `1 + k`, which keeps every fixed point identical but makes it attracting. `Push` uses −100 (the reference default) and `Pull` uses +40, with colour/alpha easing and a small depth parallax. Leaving the window drops the force and the cloud springs home. Sampling and cloud rebuilds are debounced by 150 ms on resize; nothing else animates the background.
