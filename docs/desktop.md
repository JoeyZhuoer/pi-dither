...
The background keeps its original idle field while stopped, shimmering in place with the same random-like dither vibration. While the model is thinking or producing output, random droplets rain onto a simulated water surface on the backdrop grid: each droplet sends out a ring, the rings superpose and interfere, and they reflect off the window edges. The surface is lit by its own slope, so the ripples appear as dithered crests and troughs; output rains more energetically than thinking. When generation stops, no further droplets are added and the surface slowly flattens back to the plain pattern. Output takes priority across connected manual agents and current-session extension children; tools/unknown busy work share the thinking rain without claiming verified model reasoning. Settled/disconnected records do not animate as active work. Activity changes do not override any pause setting.

The background animates at a capped ~12fps, with at most 100,000 drawing cells and a 2048px bitmap edge, and uses the same crisp Bayer lattice in every state. The water surface is a damped 2D wave simulation stepped at 24 passes per second, so waves travel roughly 55 px/s, interfere linearly and reflect off zero (edge) boundaries; damping makes the ripples slowly disappear (about half a minute) once generation stops, and a flat field is skipped entirely. Hidden tabs/pagehide, system reduced-motion settings and manual pause stop its animation timer. Use **? → Pause background motion** to pause; the preference persists across reloads. Reduced motion takes priority.
...
Tool choices are runtime-only, not saved defaults or transcript entries. The current selection carries through new/clone, workspace/resume and provider replacement. Restarting the server starts a fresh main with core defaults plus the supported extension's defaults, rather than restoring a saved session's historical tool selection.
...
The main agent defaults to core read/bash/edit/write plus the installed `pi-subagents` extension's tools (`subagent`, `bg_wait`, `subagent_supervisor` under its configuration). Available tools come from the SDK catalog; extension tools must have approved entry-point provenance. A manual desktop subagent window is created with only `read,grep,find,ls` (or an explicitly selected subset, including none), no delegation extension, a read-only system instruction, and its own session. Those windows cannot write. Agents launched through the main agent's `subagent` tool instead follow pi-subagents' configured permissions, limits and workflow policies; they can include writers. **This is a tool restriction, not an OS sandbox or protection against reading sensitive files.** Subagents share the working directory and may observe main-agent edits while inspecting it. For manual desktop windows there is no automatic parent/child transcript sharing or launch; Handoff inserts the latest completed assistant result into the main editor and the user decides whether to send it. Extension-delegated jobs are separate: their normal tool output and visible completion messages appear in the main conversation, and native completion may trigger a continuation. They now get separate automatic observer windows/taskbar entries, not manual numbered sessions. Its terminal FleetView, custom dialogs and inspector UI are not embedded; use the extension's text tools for status/control. Unsupported confirmation dialogs are cancelled, never automatically approved.

The SDK host sets `PI_OFFLINE=1`, creates its model runtime with network catalog refresh disabled, loads resources with `noExtensions:true`, and uses `SettingsManager.create(...,{projectTrusted:false})`. Explicit `additionalExtensionPaths` selectively load pi-subagents for main despite `noExtensions:true`; ambient and project extensions remain disabled in the desktop host. The extension's own configured child runtimes follow its normal policies. This preserves offline catalog startup and the untrusted-project host boundary, not a tool-execution approval mechanism. Global Pi provider configuration/auth and ordinary skills/prompts remain available. Custom providers implemented by disabled extensions are not supported. Model/thinking selection uses non-persisting RPC defaults.
