...
The background repaints only when the colour, photo or viewport changes (resize is debounced by 150 ms); no timers run for it. The photo bitmap is **one cell per CSS pixel** (dots about 1px, painted as a single ImageData buffer), bounded to 4,000,000 cells and a 4096px edge for huge windows, and cover-fits the window. A typical 1440×940 window dithers and paints in roughly 40–70 ms once, which is why no animation uses that grid. Ground colour and photo persist in the local website store (`pi-desktop:ground:v1`, `pi-desktop:photo:v1`); the one-shot window-layout reset does not touch them.
