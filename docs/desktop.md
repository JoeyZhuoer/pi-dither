...
The photo is sampled cover-fit at **one cell per CSS pixel**, bounded to 4,000,000 cells and a 4096px edge, and decimated into at most **200,000** points drawn as 3px dots straight into one reused ImageData buffer (about 4.7 ms median per frame measured for a 193k-point cloud at 1440×900; the typed-array pool is ~6 MB). The only background-pointer interaction is the reference field's: each point eases toward its home with `1/speed` (speed 20–30) while the pointer adds `strength · (pointer − p) / (1 + d)²` — `Push` uses −100 (the reference default) and `Pull` uses +40 — with colour/alpha easing and a small depth parallax. Leaving the window drops the force and the cloud springs home. Sampling and cloud rebuilds are debounced by 150 ms on resize; nothing else animates the background.
