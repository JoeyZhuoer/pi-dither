...
The background keeps its original idle field while stopped, shimmering in place with the same random-like dither vibration, and it stays exactly where it is: stopping never snaps the pattern back. While the model is thinking or producing output, the dithered wave slides steadily in a single direction with no vibration of its own, and it freezes at its current position the moment work stops, where the idle shimmer resumes. Output takes priority across connected manual agents and current-session extension children; tools/unknown busy work share the thinking wave without claiming verified model reasoning. Settled/disconnected records do not animate as active work. Activity changes do not override any pause setting.

The background animates at a capped ~12fps, with at most 100,000 drawing cells and a 2048px bitmap edge, and uses the same crisp Bayer lattice in every state. Idle keeps its original slow shimmer. Busy states advance one band phase in a single direction (2.4 rad/s, roughly 0.65% of pixels changing per frame, ~21–28 px/s of travel) and hold everything else rigid, so the wave never vibrates while generating; the phase is frozen when work stops. Hidden tabs/pagehide, system reduced-motion settings and manual pause stop its animation timer. Use **? → Pause background motion** to pause; the preference persists across reloads. Reduced motion takes priority.
