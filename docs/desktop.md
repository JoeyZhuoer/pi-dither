...
Use **Windows ▾** or the taskbar to show a feature. The **⚙** button at the top right is the window chooser: it lists every window, lets you tick which ones stay in the Windows menu (less frequent windows start out) and opens any window directly; the choice persists in `pi-desktop:windows:v1`. Utility close/minimize hides it without discarding its inputs. The separate Window Manager utility has been removed. Title-bar controls, the taskbar and Arrange still manage windows. Hiding never stops an agent.
