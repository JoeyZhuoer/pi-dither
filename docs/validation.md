...
The stopped background is byte-identical to the original idle field (SHA-256 golden hashes at several times). While an agent is thinking or producing output, the whole field glides slowly and continuously: idle shimmer changes ~0.15–0.2% of cells per frame, busy states ~1–2% per frame, moving about 1.6 cells/second along a bounded canvas-relative path. The threshold is sampled in the same moving coordinates (continuous interpolation of the 4×4 lattice), so the field is a coherent translation of itself and whole-cell crossings do not produce re-dither pops; a 24-frame sweep asserts no frame changes more than a small fraction. Unit tests assert the drift path itself (direction, bounded reach, ≤0.3 cells per frame), while pixel tests cover per-frame change, direction, smoothness and on-canvas persistence. The Chromium fixture drives synthetic idle/thinking/output activity through the real `aggregateActivity` -> `setActivity` wiring and captures each pattern; the native smoke verifies idle reporting and the motion preference. No provider prompt or paid call was used. See [feature audit](native-feature-audit.md).
