...
The animated background pattern is removed: the old backdrop module, canvas, motion preference and pause control are gone, and aggregated fleet activity is still computed and exposed as `#desktop[data-activity]` for the Activity window. In its place the appearance is user-controlled from the **Appearance** window: a theme (chrome) colour, a ground colour and an image picker, each with its own default. The photo is downscaled, stored locally and rendered as the **point-cloud background**: sampled cover-fit at one cell per CSS pixel, decimated into at most **200,000** points (3px dots written into a reused ImageData buffer) packed as `x, y, z, r, g, b, a`, and drawn with the reference field's spring plus a signed `1/(1+d)²` force — **Cloud pointer** selects Push (−100, the reference default) or Pull (+40) — with colour/alpha easing, a small depth parallax drawn centred on the canvas with Y-up cloud space (regression guard: the fixture and the native check both assert the cloud's ink centroid sits on the canvas centre, and the native spring-back check pins the pointer outside for both samples so the parallax is off in both) and a fixed 200,000-slot typed-array pool (~6 MB) whose surplus points fade to alpha −1 (measured 192,817 points: 28.2 ms cloud build once, 4.7 ms median / 5.3 ms p90 frame time in headless Chromium). That is the only background-pointer interaction; leaving the window drops the force and the cloud springs home. Unit tests cover colour normalization, one-cell-per-CSS-pixel sampling with the large-window bound, ordered-dither point selection, cloud decimation/centring/photo colours, the exact 1/speed easing, both force signs, the settled state, surplus fade-out, parallax drawing and storage/migration guards (including the legacy photo-mode keys); the Chromium fixture and the native WKWebView smoke change both colours, feed a synthetic photo through the real file input, verify the centred point cloud grows, observe the pointer displacing it and its return to the home shape, switch push/pull, and confirm removal and both resets clear/restore storage. The optional extra particle field (off/sparse/normal/dense) is covered by its own tests: deterministic creation, bounded speed/edges, elastic collisions with flash ageing, link/draw rules and a clean clear when switched off; it has no pointer interaction. The top-right **⚙** window chooser lists every window, remembers which ones keep a bottom-bar button (less frequent windows start out) and opens any window directly; every window remains in the Windows menu. No upload, network request or animated backdrop is involved, and the one-shot window-layout reset leaves colour, cloud pointer and photo untouched.
