...
## Water-surface background (unreleased release artifact; source on `main`)

The stopped background is byte-identical to the original idle field (SHA-256 golden hashes at several times) and keeps its random-like dither vibration in place. While an agent is thinking or producing output, random droplets rain onto a damped 2D water simulation on the backdrop grid: each droplet expands into a ring, the rings superpose linearly (interference is verified by comparing two droplets together with the same droplets simulated separately), and the surface reflects off zero (edge) boundaries — a closed surface keeps its energy over 2000 passes, so waves bounce instead of leaking. Ripples are lit by the surface slope, so they render as dithered crests and troughs (a single droplet changes ~1% of cells, a busy field 3–6%). When generation stops no droplets are added and the damping flattens the surface to exactly the plain pattern (verified by frame equality after the field reports itself silent); a flat field is skipped entirely. Unit tests pin idle goldens and shimmer, superposition, reflective energy, bounded peaks, damping to silence, signal-free controller rain, invisibility while idle and the fade-to-plain equality. A small controller bookkeeping note: the background owns the surface, so `drawBackdrop` stays pure and the browser fixture and native smoke exercise the same wiring. No provider prompt or paid call was used. See [feature audit](native-feature-audit.md).
