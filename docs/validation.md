...
The background pattern is removed: the app no longer ships or serves a backdrop canvas/module, draws no pixels behind the windows, and has no motion preference or pause control. Aggregated fleet activity is still computed and exposed as `#desktop[data-activity]` for the Activity window; the browser fixture drives synthetic idle/thinking/output phases to prove that wiring. Unit/browser/native checks assert no `#backdrop`/`#background-motion` exists and that no pattern animation runs.
