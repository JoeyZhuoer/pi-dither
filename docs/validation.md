...
The animated background pattern is removed: the old backdrop module, canvas, motion preference and pause control are gone, and aggregated fleet activity is still computed and exposed as `#desktop[data-activity]` for the Activity window. In its place the desktop ground is user-controlled from the **Background** window: a colour input plus an image picker. The photo is downscaled, stored locally and painted as an ordered dither between the ground colour and the standard dark ink; "Remove photo" and "Default colour" restore the plain ground. Unit tests cover colour normalization, bounded bitmap sizing, deterministic ordered dithering by luminance, ground-then-ink painting and storage guards; the Chromium fixture and the native WKWebView smoke change the colour, feed a synthetic photo through the real file input, verify ink appears on the canvas, and verify removal and reset clear storage. No upload, network request or animated backdrop is involved, and the one-shot window-layout reset leaves colour and photo untouched.
