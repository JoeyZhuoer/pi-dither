...
The stopped background is byte-identical to the original idle field (SHA-256 golden hashes at several times) and keeps its random-like dither vibration in place; idle pixels never travel. While an agent is thinking or producing output, one band phase advances steadily in a single direction (2.4 rad/s; both busy modes demodulate to a monotone directional phase, ~0.65% of pixels changing per frame, ~21–28 px/s). Everything else stays rigid, so a busy background is time-invariant: the same phase renders the same pixels at any animation time, which is what makes generating "without vibration". The animation loop owns the phase, so stopping freezes the pattern at its current position and the idle shimmer resumes there without snapping back; resuming continues the same one-way travel. Unit tests pin idle goldens, busy time-invariance, demodulated direction, per-frame change with no re-dither pop, on-canvas phases, signal-free phase accumulation, the freeze-on-stop/reference equality and idle shimmer. The Chromium fixture drives synthetic idle/thinking/output activity through the real `aggregateActivity` -> `setActivity` wiring and captures each pattern; the native smoke checks the idle report. No provider prompt or paid call was used. See [feature audit](native-feature-audit.md).
