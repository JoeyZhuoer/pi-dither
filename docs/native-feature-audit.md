...
| Flowing background while busy | Backdrop unit tests (byte-identical idle goldens, text-seeded wave determinism/shape/travel/progress, small per-frame drift, no re-dither pop, bounded path); Chromium fixture text-seed wiring and captures; native idle reporting | Pass; idle stays as before, busy shows a text-shaped dither wave; live provider streaming not exercised |
