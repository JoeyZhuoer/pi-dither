...
| Appearance colours, dithered photo, reference-style pointer stir (26px swirl + 90px links, ~1s recovery) | Unit tests (colour validation, pixel-precise bounded bitmap, ordered-dither luminance, exact ImageData pixels, ripple spread/settle/region math, storage guards); Chromium and native WKWebView fixtures (both colours apply, synthetic photo dithers to ink, remove/default restore, pointer move spreads and settles to the exact base) | Pass; photos stay in the local website store and are never uploaded; live file-dialog interaction not automated |
