...
1. **Sizing was packaged, but its state was ambiguous.** Before this fix, installed and source `windows.js` had identical SHA-256 digests. A single `zoomed` flag represented both a prior automatic fit and a deliberate manual adjustment. Reproducing a narrow-to-wide desktop transition showed why an auto-fitted window could remain pinned to its smaller size.
   ...
   - New visible main/child/observer windows fit immediately, not only after title-bar selection. Utilities fit when opened. Separate Auto-size/↗ controls and the native sizing menu command have been removed at the user's request. Children retain their right-side anchors and fit the available height below them.
   - Old `zoomed=true` layouts remain protected because their origin cannot be inferred. Arrange resets sizing intent to compact defaults, which fit on selection/opening. An explicitly requested native `--reset-window-layout` clears only saved panel geometry/visibility, observer dismissals and the native frame. It is one-shot; Reload preserves subsequent manual geometry and unrelated preferences.
...
| --- | --- | --- |
| Automatic-on-open fitting, purpose presets, native resizing | Unit tests; real DOM; native 800/980/1440px width transitions | Pass; no fitting button, distinct purpose sizes |
...
| Flowing background while busy | Backdrop unit tests (byte-identical idle goldens, exact whole-cell translation, bounded path); Chromium fixture activity wiring and captures; native idle reporting | Pass; idle vibrates in place, busy translates the whole field; live provider streaming not exercised |
