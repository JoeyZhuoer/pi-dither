...
| Particle field | Unit tests (deterministic field, bounded speed/edges, elastic collisions + flash ageing, pointer reach, link/draw rules); Chromium + native fixtures (densities draw, pointer stirs, off clears) | Pass; optional and off by default; hidden tab pauses, reduced motion is static |
