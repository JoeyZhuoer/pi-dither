...
| Flowing background while busy | Backdrop unit tests (byte-identical idle goldens, small per-frame drift, no re-dither pop, bounded path); Chromium fixture activity wiring and captures; native idle reporting | Pass; idle stays as before, busy glides gently; live provider streaming not exercised |
